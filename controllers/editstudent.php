                    ...
                    <div class="relative w-fit flex flex-wrap -mt-8 mx-10">
                        <div class="flex flex-row justify-start items-center space-x-2 uppercase text-blueGray-600 text-xs font-bold ">
                            ...
                            while ($resp = mysqli_fetch_assoc($run)) {
                                ...
                                $cour_query = "SELECT courseName FROM studentscourse WHERE stud_id = $ids";
                                $runque = mysqli_query($conn,$cour_query);
                               
                                print_r($respc = mysqli_fetch_array($runque));
                                exit;
                            ...
                            ?>
                                ...
                                <label class="text-[0.8rem]"><input type="Checkbox" name="course[]" id="react" value="<?php echo $resp['courseName']; ?>"
                                <?php
                                    if ($respc['courseName'] == $resp['courseName']) {
                                        echo "checked";
                                    }
                                    
                                ?>
                                /> <?php echo $resp['courseName']; ?></label>
...
if (isset($_POST['stupdate'])) {
    ...
    if (isset($_POST['course'])) {
        ...
        foreach ($_POST['course'] as $value) {
            mysqli_query($conn, "insert into studentscourse (stud_id, courseName) values($ids,'$value')");
        }
